 ...
 */
//这个类把正则语法做了一个详细的解析，包括存储字母表，存储表达式
public class RegularGrammar {
    public ArrayList<Character> symbols = new ArrayList<Character>();//the alphabet for regular grammar，也就是字母表
    private final ArrayList<Regex> patterns = new ArrayList<>();//也就是把语法式子再存一次
    public RegularGrammar(String[] regexes){//支持读入多个正则语法
        for(String r: regexes){
            ...
            regex = stringUtil.trim(regex);//打出名字和对应的正则语法句子

            Regex p = new Regex(name, regex, 0);
            this.patterns.add(p);
            for(Character ch : regex.toCharArray()){//Character是用于对char操作的类，isLetter识别其是不是一个字母
